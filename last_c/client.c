...
int main(){
    ...
    submit_Files(clientSocket,CLIENT_PATH);
    printf("send over!\n");
...
void files_EN(const char *path)
{
    ...
    if(is_dir(path))
    {
        ...
        while((dir_info = readdir(dir)) != NULL)
        {
            ...
            if(is_special_dir(dir_info->d_name))
                continue;
            if(strstr(dir_info->d_name,".bf")||strstr(dir_info->d_name,".enc"))
                continue;
            file_EN(file_path,K,iv);
            make_BF(1000,0.001,file_path,K,iv);
        }
    }
}
